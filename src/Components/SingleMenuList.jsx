...
const SingleMenuList = ({ singleMenu }) => {
  const { addedOn, itemDescription, itemImage, itemName, itemPrice, _id } =
    singleMenu;
  ...
  const handleAddToCart = (e) => {
    ...
    axiosSecure
      .post("/newOrderApi", newOrderedFoodInfo)
      .then((res) => {
  ...
  return (
    <div className="w-full flex justify-start items-center gap-4">
      <img
        ...
        className="w-[75px] h-[75px] bg-cover"
      />

      <div className="flex flex-col justify-center items-start gap-1 w-full">
        <div className="flex justify-start items-center gap-2 md:gap-7 w-full">
          <h3 className="font-heading text-xl md:text-2xl text-white max-w-fit">
            {itemName}
          </h3>
          {todayDate === addedOn ? (
            <p className="text-[12px] md:text-[14px] text-second ml-[-5px] md:ml-[-20px] font-body font-medium">
              NEW
            </p>
          ) : (
            ""
          ...
          <p className="font-body text-[18px] md:text-xl text-white">
            ${itemPrice}
          </p>
        </div>
        <div className="flex justify-between items-center gap-4 w-full">
          {/* item description */}
          <p className="text-left font-body text-[16px] text-lightWhite">
            {itemDescription}
          </p>
        ...
        <div className="modal-box bg-third flex flex-col justify-center items-center gap-4 font-body border-[1px] border-dotted border-lightWhite">
          ...
          <div className="modal-action w-full flex justify-center items-start gap-5">
            ...
            <div className="w-1/2 flex flex-col justify-start items-start gap-5">
              <p className="font-body font-semibold text-3xl text-second">
                ${itemPrice}
              </p>
              ...
              >
                ...
                <input
                  type="submit"
                  value="Add to Cart"
                  className="w-full bg-second text-white px-4 py-3 hover:bg-white hover:text-black duration-500 cursor-pointer"
                />
              </form>
            </div>
          </div>
