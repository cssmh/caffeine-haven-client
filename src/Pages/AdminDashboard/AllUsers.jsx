...
const AllUsers = () => {
  ...
  const handleBlackListUser = (id) => {
    ...
    axiosSecure
      .put(`/updateUser/${id}`, updatedUserStatus)
      .then((res) => {
        const data = res.data;
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "ID",
      cell: (row) => (
        <p className="text-center text-[16px]">{row.row.original._id}</p>
      ),
    },
    {
      accessorKey: "email",
      ...
      cell: (row) => (
        <div className="flex w-full justify-center items-center">
          <p className="capitalize text-center">
            {row.row.original.userCreationDate}
          </p>
        </div>
      ),
    },
    {
      ...
      cell: (row) => (
        <div className="flex w-full justify-center items-center">
          <p
            className={`${
              row.row.original.userStatus === "blocked"
                ? "text-[#ff2a2a]"
                : "text-[#18e429]"
            } capitalize text-center`}
          >
            {row.row.original.userStatus || "Regular"}
          </p>
        </div>
      ),
    },
    {
      ...
      cell: (row) => (
        <button
          disabled={row.row.original.userStatus === "blocked"}
          onClick={() => {
            Swal.fire({
              title: "Are you sure?",
