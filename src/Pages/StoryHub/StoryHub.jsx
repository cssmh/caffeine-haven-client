...
const StoryHub = () => {
  ...
  const handlePostNewMemory = (e) => {
    ...
    const handleDbPost = (postedImage) => {
      ...
      axiosSecure
        .post("/postNewMemoryApi", newPostInfo)
        .then((res) => {
    ...
    <div className="mx-auto overflow-x-hidden">
      ...
      >
        ...
        {user ? (
          ...
          >
            <h1 className="text-xl text-center uppercase md:text-2xl font-heading">
              Share Your Thoughts
            </h1>

            {/* message and image post */}
            {user?.userStatus === "blocked" ? (
              <p className="mt-4 text-center text-[#e65858] md:text-2xl font-body">
                You are blocked from posting anything
              </p>
            ) : (
              <form
                onSubmit={handlePostNewMemory}
                ref={postMemoryForm}
              ...
              >
                ...
                <div className="w-full flex justify-center items-center gap-2 md:gap-5">
                  ...
                  <textarea
                    required
                    name="userPost"
                    placeholder="Reveal your thoughts"
                    maxLength={250}
                    id="userPost"
                    className="w-[90%] border-[1px] border-[#ffffff00] bg-[#f0f0f021] focus:outline-none focus:border-lightBlack px-3 py-1 text-[16px] rounded-[5px]"
                  />
                  {/* image file input */}
                  <label
                    htmlFor="image"
                ...
                <div className="w-full flex justify-start items-center gap-3">
                  ...
                  <input
                    type="submit"
                    value="Share"
                    className="bg-second text-white px-4 py-1 hover:bg-white hover:text-black duration-500 cursor-pointer rounded-[3px]"
                  />
                </div>
              </form>
            )}
          ...
          <div className="flex flex-col gap-4 justify-center items-center p-5 bg-[#0e0e0e77] mt-[100px]">
            <p className="text-2xl md:text-3xl text-center font-heading text-white">
              Please sign in to share your thoughts
            </p>
            <Link to={"/signIn"}>
              <ButtonMain buttonText={"Sign in"} />
            </Link>
          </div>
        )}
      </div>
      ...
      <div className="container mx-auto p-5 flex flex-col lg:flex-row justify-center items-start gap-10 mt-10 min-h-[100vh] overflow-y-auto">
        ...
        <motion.div
          variants={rightAnimation(0.8, 0.5)}
          initial="hidden"
          whileInView={"visible"}
        ...
        >
          ...
          <div className="w-full grid grid-cols-4 md:grid-cols-6 lg:grid-cols-2">
            {allMemories.map((memory, index) => (
              <PinnedMemory key={index} memory={memory}></PinnedMemory>
            ))}
          </div>
        </motion.div>
        ...
        <div className="w-[98%] lg:w-4/5 flex flex-col justify-start items-start gap-14">
          {allMemories.map((memory, index) => (
            <SingleMemory
              key={index}
              memory={memory}
              memoriesRefetch={memoriesRefetch}
            ></SingleMemory>
          ))}
        </div>
      </div>
