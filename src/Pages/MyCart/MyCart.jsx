...
const MyCart = () => {
  ...
  const handleRemoveFromCart = (id) => {
    axiosSecure
      .delete(`/deleteItemFromCartApi/${id}`)
      .then((res) => {
  ...
  const handleApplyCoupon = (e) => {
    ...
    axiosSecure
      .post("/couponCodeValidationApi", applyCouponInfo)
      .then((res) => {
        const data = res.data;
        if (data.coupon) {
  ...
  const columns = [
    {
      ...
      cell: (row) => (
        <button
          onClick={() => handleRemoveFromCart(row.row.original._id)}
          className="w-fit text-xl text-[#ff4747] hover:text-[#bd2a2a] duration-300"
        >
          <MdCancel />{" "}
        </button>
      ),
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex justify-center items-center">
          <img
            src={row.row.original.foodImage}
            alt="Food image"
    ...
    },
    {
      ...
      header: "Item name",
      cell: (row) => (
        <p className="text-center text-[16px]">{row.row.original.foodName}</p>
      ),
    },
    {
      ...
      header: "Price",
      cell: (row) => (
        <p className="text-center text-[16px]">${row.row.original.foodPrice}</p>
      ),
    },
    {
      accessorKey: "",
      ...
      cell: (row) => (
        <p className="text-center text-[16px]">
          {row.row.original.foodQuantity} nos
        </p>
      ),
    },
    {
      ...
      cell: (row) => (
        <p className="text-center text-[16px]">
          ${" "}
          {parseFloat(
            row.row.original.foodQuantity * row.row.original.foodPrice
          ).toFixed(2)}
        </p>
      ),
    },
  ...
  return (
    <div className="mx-auto">
      ...
      {cartItems.length === 0 ? (
        <p className="py-[150px] text-3xl text-center font-body text-lightWhite">
          No items on cart
        </p>
      ) : (
        <>
          ...
          <div className="container mx-auto my-5 md:my-7 lg:my-10 w-full flex flex-col justify-center items-center gap-7 p-5">
              ...
              >
                <thead>
                  {table.getHeaderGroups().map((headerGroup, index) => (
                    <tr
                      key={index}
                      className="table-row text-second font-semibold"
                    >
                      {headerGroup.headers.map((header) => (
                        <th
                          key={header?.id}
                          className="table-description text-sub"
                        >
                          {flexRender(
                  ...
                  {table.getRowModel().rows.map((row, index) => (
                    <tr key={index} className="table-row">
                      {row.getVisibleCells().map((cell, index) => (
                        <td
                          key={index}
                          className="text-center table-description"
                        >
                          {flexRender(
              ...
              <div className="w-full flex justify-end items-center">
                ...
                >
                  ...
                  <input
                    ...
                    id="couponCode"
                    className="focus:outline-none bg-black px-3 md:px-5 py-2 md:py-3 self-stretch font-body"
                  />
                  {/* submit button */}
                  <input
                    type="submit"
                    value="Apply Coupon"
                    className="bg-second px-3 md:px-5 py-2 md:py-3 font-heading uppercase cursor-pointer hover:bg-white hover:text-black duration-500 self-stretch"
                  />
                </form>
              </div>
            </div>
            ...
            >
              <h3 className="text-3xl font-heading uppercase text-white">
                Cart Total
              </h3>
              ...
              <div className="w-full font-body flex flex-col md:flex-row justify-center items-center gap-5 mt-5">
                ...
                <div className="w-full md:w-1/2 flex justify-between items-center border-[1px] border-lightBlack border-dotted p-3 self-stretch">
                  <p className="text-2xl">
                    Total{" "}
                    <span className="text-[18px] text-[#ff3b3b]">
              ...
              <Link
                ...
                className="w-full bg-second py-3 text-xl md:py-4 font-heading uppercase text-center mt-3 hover:bg-white hover:text-black duration-500 font-medium"
              >
                Proceed to checkout
              </Link>
            </motion.div>
