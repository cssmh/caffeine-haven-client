...
const Coupons = () => {
  ...
  const handleCreateNewCoupon = (e) => {
    ...
    axiosSecure
      .post("/newCouponCreateApi", newCouponInfo)
      .then((res) => {
        const data = res.data;
        if (data.insertedId) {
          couponsRefetch();
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "ID",
      cell: (row) => (
        <p className="text-center text-lightWhite text-[16px]">
    ...
    },
    {
      ...
      header: "Discount",
      cell: (row) => (
        <p className="text-center">{row.row.original.discountPercentage}%</p>
      ),
    },
    {
      accessorKey: "",
      ...
      cell: (row) => (
        <button
          ...
          className="bg-[#af0606] disabled:bg-[#694d4d] disabled:cursor-not-allowed px-2 text-[16px] uppercase hover:bg-third duration-500 py-1"
        >
          Delete
        </button>
      ),
  ...
  return (
    <div className="container mx-auto mt-5 md:mt-7 lg:mt-10 w-full flex flex-col justify-center items-center gap-7">
      ...
      <div className="w-full flex justify-end items-center">
        <button
          onClick={() =>
            document.getElementById("couponAddingModal").showModal()
          }
          className="bg-second text-white hover:bg-main hover:text-white duration-500 font-heading font-medium uppercase px-4 py-3"
        >
          Add New Coupon
        </button>
      </div>
      ...
      {/* coupon adding modal */}
      <dialog
        id="couponAddingModal"
        className="modal modal-bottom sm:modal-middle py-5"
      >
        <div className="modal-box bg-third flex flex-col justify-center items-center gap-4 font-body">
          ...
          <div className="modal-action w-full">
            <form
              onSubmit={handleCreateNewCoupon}
              method="dialog"
            ...
            >
              ...
              <input
                ...
                className="w-full md:w-3/4 bg-main text-lightWhite px-4 py-3 focus:outline-none"
              />
              ...
              <input
                type="submit"
                value="Create Coupon"
                className="w-full md:w-3/4 bg-second text-white px-4 py-3 hover:bg-white hover:text-black duration-500 cursor-pointer"
              />
            </form>
          </div>
        </div>
